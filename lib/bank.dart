...
class Bank {
 ...
 BankAccount? findAccount(double accountNumber){
  ...
  print("=== Bank Account Report ===");
  for (var acc in _accounts) {
    print("Account Number: ${acc.accountNumber}, Holder: ${acc.getAccHolderName()}, Balance: \$${acc.balance.toStringAsFixed(2)}");
    if (acc.transactions.isNotEmpty) {
      print("Transactions:");
      for (var tx in acc.transactions) {
        print("  - $tx");
      }
    } else {
  ...
  for (var acc in _accounts) {
    if (acc is InterestBearing) {
      ...
      acc.addTransaction("Interest applied: \$${interest.toStringAsFixed(2)}");
      print("Applied \$${interest.toStringAsFixed(2)} interest to ${acc.getAccHolderName()}");
    }
  }
  print("================================");
}
